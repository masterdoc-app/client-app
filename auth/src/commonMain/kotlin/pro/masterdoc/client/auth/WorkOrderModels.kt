...
class WorkOrdersRepository(
    ...
    suspend fun siteWorkOrders(siteId: String, from: String, to: String): List<WorkOrderDto> {
        val response =
            http.get(
                url = "${base()}/reports/site-work-orders?siteId=$siteId&from=$from&to=$to",
                headers = mapOf("Authorization" to "Bearer ${bearer()}"),
            )
        if (!response.isSuccessful) {
            throw GatewayHttpException(
                response.status,
                response.body.ifBlank { "site work orders report failed" },
            )
        }
        return json.decodeFromString(response.body)
    }
    ...
    suspend fun timeToFirstAction(from: String, to: String): List<WorkOrderDto> {
        val response =
            http.get(
                url = "${base()}/reports/time-to-first-action?from=$from&to=$to",
                headers = mapOf("Authorization" to "Bearer ${bearer()}"),
            )
        if (!response.isSuccessful) {
            throw GatewayHttpException(
                response.status,
                response.body.ifBlank { "time to first action report failed" },
            )
        }
        return json.decodeFromString(response.body)
    }

    suspend fun pprPlanFact(from: String, to: String): List<WorkOrderDto> {
        val response =
            http.get(
                url = "${base()}/reports/ppr-plan-fact?from=$from&to=$to",
                headers = mapOf("Authorization" to "Bearer ${bearer()}"),
            )
        if (!response.isSuccessful) {
            throw GatewayHttpException(
                response.status,
                response.body.ifBlank { "ppr plan fact report failed" },
            )
        }
        return json.decodeFromString(response.body)
    }

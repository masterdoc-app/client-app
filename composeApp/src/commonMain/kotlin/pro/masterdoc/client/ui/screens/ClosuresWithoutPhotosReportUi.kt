...
@Composable
internal fun ClosuresWithoutPhotosReportScreen(
    ...
    val catalogItem = reportCatalogItems().first { it.id == ReportId.ClosuresWithoutPhotos }
    ...
    LaunchedEffect(reportsRepository, days) {
        ...
        try {
            orders =
                reportsRepository.closuresWithoutPhotos(
        ...
        LazyColumn(
            ...
            when {
                ...
                orders.isEmpty() ->
                    item {
                        AppText(
                            text = "Нет закрытий без фото за выбранный период",
                    ...
                    items(orders, key = { it.id }) { order ->
                        Column(
                            ...
                            AppText(
                                text = formatClosuresWithoutPhotosAssigneeLabel(order.assigneeId, users),
                                style = AppTextStyle.Label,
                            )
                        }
                    }
...
private fun formatClosuresWithoutPhotosAssigneeLabel(assigneeId: String?, users: List<AdminUser>): String {
